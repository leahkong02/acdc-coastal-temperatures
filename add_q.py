...
def the_model(F, T_ocean, theta_ocean_0, theta_land_0, theta_ft_0,
              q_ocean, q_land):
...
##################################################################################
    ...
    r_ls = 1000                    # land latent resistance [s/m]
    r_ss = 250                    # land sensible resistance [s/m]
    r_lo = 1000                    # ocean latent resistance [s/m]
    r_so = 50                    # ocean sensible resistance [s/m]

    ######## BOUNDARY LAYER PARAMETERS

    h_o = 1000                     # ocean boundary layer depth [m]
    h_l = 1000                     # land boundary layer depth [m]
    h_ft = 5000                    # free troposphere depth [m]

    tau_o_ft = 86400               # ocean BL - FT mixing timescale [s]
    #tau_l_ft = 86400               # land BL - FT mixing timescale [s]
    tau_o_l = 86400                # ocean - land exchange timescale [s]
    w_e = 0.005                    # entrainment rate [m/s]
    q_ft = 0.003                   # free troposphere specific humidity

    ######## LAND SURFACE

    theta = 0.45                   # maximum volumetric soil moisture
    ...
    theta_ocean[0] = theta_ocean_0[0]
    theta_land[0] = theta_land_0[0]
    theta_ft[0] = theta_ft_0[0]
    ...
    while i < N - 1:
        ...
        dqo_dt = (q_land[i] - q_ocean[i])/tau_o_l + E_ocean[i] + (q_ft - q_ocean[i])*w_e/h_o
        
        ########### LAND SPECIFIC HUMIDITY DYNAMICS
        
        dql_dt = (q_ocean[i] - q_land[i])/tau_o_l + E_land[i] + (q_ft - q_land[i])*w_e/h_l
...
# Surface radiative forcing [W/m2]
F = np.full(N, 250.0)

# Prescribed ocean surface temperature [K]
T_ocean = np.full(N, 286.5)

# Initial potential temperatures [K]
theta_ocean_0 = np.full(N, 283.0)
theta_land_0 = np.full(N, 280.0)
theta_ft_0 = np.full(N, 275.0)

# Specific humidity over ocean and land [kg/kg]
q_ocean = np.full(N, 0.008)
q_land = np.full(N, 0.005)

Ts,ms,thetao,thetal,P = the_model(F, T_ocean, theta_ocean_0, theta_land_0, theta_ft_0, q_ocean, q_land)

plt.plot(np.arange(len(Ts)), Ts, label = 'Ts')
#plt.plot(np.arange(len(Ts)), ms)
plt.plot(np.arange(len(Ts)), thetao, label = 'ThetaO')
plt.plot(np.arange(len(Ts)), thetal, label = 'ThetaL')
plt.legend()
plt.savefig('ACDC_test_W.png')
plt.close()


# Prescribed ocean surface temperature [K]
T_ocean = np.full(N, 296.0)
